...
B48DatabaseManager::B48DatabaseManager(const std::string &db_path)
...
bool B48DatabaseManager::add_persistent_message(
    int priority, int line_number, int tarif_zone,
...
bool B48DatabaseManager::update_persistent_message(
    int message_id, int priority, bool is_enabled,
...
bool B48DatabaseManager::bootstrap_default_messages() {
  ...
  // Instead of inserting all 4 messages, only insert 2 to reduce processing time
  // This should help avoid watchdog timeouts
  bool success = true;

  // Add messages one by one with yields between operations to prevent watchdog timeout
  ESP_LOGD(TAG, "Adding bootstrap message 1/2...");
  success &= add_persistent_message(50, 48, 101, "Base48", "Podporuj svuj mistni hackerspace!", "Loading", 0, "Bootstrap");
  esp_task_wdt_reset(); // Reset watchdog timer
  
  ESP_LOGD(TAG, "Adding bootstrap message 2/2...");
  success &= add_persistent_message(50, 49, 101, "Base48", "Support your local hackerspace!", "Loading", 0, "Bootstrap");
  esp_task_wdt_reset(); // Reset watchdog timer
  
  // Skip adding the other 2 messages for now
  // We can add more messages later when the system is stable
